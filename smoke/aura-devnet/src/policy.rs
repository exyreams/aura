...
fn treasury_pda(payer: &Keypair, agent_id: &str) -> Pubkey {
    pda(&[b"treasury", payer.pubkey().as_ref(), agent_id.as_bytes()], &ID).0
}
...
/// Run the full confidential cycle: encrypt inputs → configure guardrails →
/// propose → wait for output ciphertext → request decryption → wait for
/// plaintext → confirm decryption.
///
/// Returns the treasury domain after `confirm_policy_decryption` so callers
/// can inspect `pending.decision`.
...
async fn scenario_per_tx_deny(
    ...
    let (treasury, created_at) = setup_treasury(rpc, payer, &agent_id, aura_policy::PolicyConfig {
        per_tx_limit_usd: 500,
        ..Default::default()
    })?;
    let domain = run_confidential_cycle(rpc, payer, treasury, encrypt_program, ep,
        10_000, 500, 0, 800, created_at, 0).await?;
...
async fn scenario_per_tx_approve(
    ...
    send_tx(rpc, payer, vec![register_dwallet_ix(payer, treasury, live, created_at + 1)], &[])?;

    let domain = run_confidential_cycle(rpc, payer, treasury, encrypt_program, ep,
        10_000, 500, 0, 250, created_at, 2).await?;
    ...
    finalize_via_dwallet(rpc, payer, &mut dw, treasury, dwallet_program, live, created_at + 20).await
}
...
async fn scenario_daily_deny(
    ...
    let domain = run_confidential_cycle(rpc, payer, treasury, encrypt_program, ep,
        1_000, 5_000, 800, 400, created_at, 0).await?;
...
async fn scenario_daily_approve(
    ...
    let domain = run_confidential_cycle(rpc, payer, treasury, encrypt_program, ep,
        1_000, 5_000, 200, 300, created_at, 2).await?;
...
async fn scenario_cancel_pending(
    ...
    let (treasury, created_at) = setup_treasury(rpc, payer, &agent_id, aura_policy::PolicyConfig::default())?;
    ...
    send_tx(rpc, payer, vec![solana_sdk::instruction::Instruction {
        program_id: ID,
        accounts: accounts::ConfigureConfidentialGuardrails {
            owner: payer.pubkey(), treasury,
            daily_limit_ciphertext: daily_ct,
            per_tx_limit_ciphertext: per_tx_ct,
            spent_today_ciphertext: spent_ct,
        }.to_account_metas(None),
        data: instruction::ConfigureConfidentialGuardrails { now: created_at + 1 }.data(),
    }], &[])?;

    // Propose (only needs to exist, not wait for FHE)
    let policy_output = Keypair::new();
    let mut metas = accounts::ProposeConfidentialTransaction {
        ai_authority: payer.pubkey(), treasury,
        daily_limit_ciphertext: daily_ct, per_tx_limit_ciphertext: per_tx_ct,
        spent_today_ciphertext: spent_ct, amount_ciphertext: amount_ct,
        policy_output_ciphertext: policy_output.pubkey(),
        encrypt_program: *encrypt_program,
        config: ep.config_pda, deposit: ep.deposit_pda,
        caller_program: ID, cpi_authority: ep.cpi_authority,
        network_encryption_key: ep.network_key_pda, event_authority: ep.event_authority,
        system_program: SYSTEM_PROGRAM_ID,
    }.to_account_metas(None);
    mark_account_meta_signer(&mut metas, policy_output.pubkey())?;
    send_tx(rpc, payer, vec![solana_sdk::instruction::Instruction {
        program_id: ID, accounts: metas,
        data: instruction::ProposeConfidentialTransaction {
            args: ProposeConfidentialTransactionArgs {
                amount_usd: 200, target_chain: 2, tx_type: 0,
                protocol_id: None, current_timestamp: created_at + 2,
                expected_output_usd: None, actual_output_usd: None,
                quote_age_secs: None, counterparty_risk_score: None,
                recipient_or_contract: payer.pubkey().to_string(),
            },
        }.data(),
    }], &[&policy_output])?;

    ensure!(fetch_treasury_domain(rpc, &treasury)?.pending.is_some(), "[5] pending should exist");

    // Cancel
    send_tx(rpc, payer, vec![solana_sdk::instruction::Instruction {
        program_id: ID,
        accounts: accounts::CancelPending { owner: payer.pubkey(), treasury }.to_account_metas(None),
        data: instruction::CancelPending { now: created_at + 3 }.data(),
    }], &[])?;

    ensure!(fetch_treasury_domain(rpc, &treasury)?.pending.is_none(), "[5] pending should be cleared");
    println!("  ✓ cancelled");
    Ok(())
}
...
async fn scenario_pause_resume(
    ...
    send_tx(rpc, payer, vec![solana_sdk::instruction::Instruction {
        program_id: ID,
        accounts: accounts::PauseExecution { owner: payer.pubkey(), treasury }.to_account_metas(None),
        data: instruction::PauseExecution { paused: true, now: created_at + 1 }.data(),
    }], &[])?;
    ensure!(fetch_treasury_domain(rpc, &treasury)?.execution_paused, "[6] should be paused");
    println!("  ✓ paused");
    ...
    let paused_result = send_tx(rpc, payer, vec![solana_sdk::instruction::Instruction {
        program_id: ID, accounts: metas,
        data: instruction::ProposeConfidentialTransaction {
            args: ProposeConfidentialTransactionArgs {
                amount_usd: 200, target_chain: 2, tx_type: 0,
                protocol_id: None, current_timestamp: created_at + 3,
                expected_output_usd: None, actual_output_usd: None,
                quote_age_secs: None, counterparty_risk_score: None,
                recipient_or_contract: payer.pubkey().to_string(),
            },
    ...
    ensure!(paused_result.is_err(), "[6] propose should fail while paused");
    ...
    send_tx(rpc, payer, vec![solana_sdk::instruction::Instruction {
        program_id: ID,
        accounts: accounts::PauseExecution { owner: payer.pubkey(), treasury }.to_account_metas(None),
        data: instruction::PauseExecution { paused: false, now: created_at + 4 }.data(),
    }], &[])?;
    ensure!(!fetch_treasury_domain(rpc, &treasury)?.execution_paused, "[6] should be resumed");
    println!("  ✓ resumed");
    ...
    send_tx(rpc, payer, vec![solana_sdk::instruction::Instruction {
        program_id: ID, accounts: metas2,
        data: instruction::ProposeConfidentialTransaction {
            args: ProposeConfidentialTransactionArgs {
                amount_usd: 200, target_chain: 2, tx_type: 0,
                protocol_id: None, current_timestamp: created_at + 5,
                expected_output_usd: None, actual_output_usd: None,
                quote_age_secs: None, counterparty_risk_score: None,
                recipient_or_contract: payer.pubkey().to_string(),
            },
    ...
    ensure!(fetch_treasury_domain(rpc, &treasury)?.pending.is_some(), "[6] proposal should exist after resume");
    ...
    send_tx(rpc, payer, vec![solana_sdk::instruction::Instruction {
        program_id: ID,
        accounts: accounts::CancelPending { owner: payer.pubkey(), treasury }.to_account_metas(None),
        data: instruction::CancelPending { now: created_at + 6 }.data(),
    }], &[])?;
    Ok(())
}
...
async fn scenario_multisig_override(
    ...
    let g3 = Keypair::new();
    for g in [&g1, &g2, &g3] {
        let sig = rpc.request_airdrop(&g.pubkey(), 10_000_000)?;
    ...
    send_tx(rpc, payer, vec![solana_sdk::instruction::Instruction {
        program_id: ID,
        accounts: accounts::ConfigureMultisig { owner: payer.pubkey(), treasury }.to_account_metas(None),
    ...
    ensure!(fetch_treasury_domain(rpc, &treasury)?.multisig.is_some(), "[7] multisig not attached");
    println!("  ✓ 2-of-3 multisig configured (daily_limit=500)");

    // 600 USD should be denied under limit 500
    let domain_deny = run_confidential_cycle(rpc, payer, treasury, encrypt_program, ep,
        500, 5_000, 0, 600, created_at, 3).await?;
    ensure!(!domain_deny.pending.context("[7] no pending")?.decision.approved, "[7] should be denied before override");
    println!("  ✓ 600 USD denied under original limit");
    execute_denied(rpc, payer, treasury, created_at + 30)?;

    // g1 proposes override to 2 000
    send_tx(rpc, &g1, vec![solana_sdk::instruction::Instruction {
        program_id: ID,
        accounts: accounts::ProposeOverride { guardian: g1.pubkey(), treasury }.to_account_metas(None),
        data: instruction::ProposeOverride { new_daily_limit_usd: 2_000, now: created_at + 40 }.data(),
    }], &[])?;
    println!("  ✓ g1 proposed override (1/2 signatures)");

    // g2 collects — quorum reached, override applied immediately
    send_tx(rpc, &g2, vec![solana_sdk::instruction::Instruction {
        program_id: ID,
        accounts: accounts::CollectOverrideSignature { guardian: g2.pubkey(), treasury }.to_account_metas(None),
        data: instruction::CollectOverrideSignature { now: created_at + 41 }.data(),
    }], &[])?;

    let domain_override = fetch_treasury_domain(rpc, &treasury)?;
    ensure!(domain_override.policy_config.daily_limit_usd == 2_000, "[7] daily limit should be 2 000");
    ensure!(domain_override.multisig.as_ref().map(|m| m.pending_override.is_none()).unwrap_or(false),
        "[7] pending override should be cleared");
    println!("  ✓ quorum reached, daily limit raised to 2 000");

    // 600 USD should now be approved
    let mut dw = connect_dwallet_client().await?;
    let domain_approve = run_confidential_cycle(rpc, payer, treasury, encrypt_program, ep,
        2_000, 5_000, 0, 600, created_at, 50).await?;
    ensure!(domain_approve.pending.context("[7] no pending")?.decision.approved, "[7] should be approved after override");
    println!("  ✓ 600 USD approved under raised limit");
    finalize_via_dwallet(rpc, payer, &mut dw, treasury, dwallet_program, live, created_at + 80).await
}
...
async fn scenario_swarm_pool(
    ...
    let (treasury, created_at) = setup_treasury(rpc, payer, &agent_id, aura_policy::PolicyConfig {
        daily_limit_usd: 10_000, per_tx_limit_usd: 10_000,
        daytime_hourly_limit_usd: 10_000, nighttime_hourly_limit_usd: 10_000,
        velocity_limit_usd: 10_000, shared_pool_limit_usd: Some(300),
        ..Default::default()
    })?;
    send_tx(rpc, payer, vec![register_dwallet_ix(payer, treasury, live, created_at + 1)], &[])?;
    ...
    send_tx(rpc, payer, vec![solana_sdk::instruction::Instruction {
        program_id: ID,
        accounts: accounts::ConfigureSwarm { owner: payer.pubkey(), treasury }.to_account_metas(None),
    ...
    ensure!(fetch_treasury_domain(rpc, &treasury)?.swarm.is_some(), "[8] swarm not attached");
    println!("  ✓ swarm configured (pool_limit=300)");

    // First tx: 200 USD — within pool
    let mut dw = connect_dwallet_client().await?;
    let d1 = run_confidential_cycle(rpc, payer, treasury, encrypt_program, ep,
        10_000, 10_000, 0, 200, created_at, 3).await?;
    ensure!(d1.pending.context("[8] no pending (1)")?.decision.approved, "[8] first tx should be approved");
    println!("  ✓ first tx (200) approved");
    finalize_via_dwallet(rpc, payer, &mut dw, treasury, dwallet_program, live, created_at + 30).await?;
    println!("  ✓ finalized, pool spent=200");

    // Second tx: 200 USD — pool has 100 remaining, should deny
    let d2 = run_confidential_cycle(rpc, payer, treasury, encrypt_program, ep,
        10_000, 10_000, 0, 200, created_at, 40).await?;
    ensure!(!d2.pending.context("[8] no pending (2)")?.decision.approved, "[8] second tx should be denied");
...
async fn scenario_single_guardian_override(
    ...
    let sig = rpc.request_airdrop(&guardian.pubkey(), 10_000_000)?;
    rpc.confirm_transaction(&sig)?;
    ...
    send_tx(rpc, &guardian, vec![solana_sdk::instruction::Instruction {
        program_id: ID,
        accounts: accounts::ProposeOverride { guardian: guardian.pubkey(), treasury }.to_account_metas(None),
        data: instruction::ProposeOverride { new_daily_limit_usd: 5_000, now: created_at + 2 }.data(),
    }], &[])?;

    let domain = fetch_treasury_domain(rpc, &treasury)?;
    ensure!(domain.policy_config.daily_limit_usd == 5_000, "[9] daily limit should be 5 000");
    ensure!(domain.multisig.as_ref().map(|m| m.pending_override.is_none()).unwrap_or(false),
...
async fn scenario_multi_chain(
    ...
    send_tx(rpc, payer, vec![solana_sdk::instruction::Instruction {
        program_id: ID,
        accounts: accounts::RegisterDwallet { owner: payer.pubkey(), treasury }.to_account_metas(None),
        data: instruction::RegisterDwallet {
            args: RegisterDwalletArgs {
                chain: 1, dwallet_id: format!("eth-mock-{seed}"),
                address: "0xdeadbeefdeadbeefdeadbeefdeadbeefdeadbeef".to_string(),
                balance_usd: 0, dwallet_account: None, authorized_user_pubkey: None,
                message_metadata_digest: None, public_key_hex: None, timestamp: created_at + 2,
            },
        }.data(),
    }], &[])?;
    ...
    send_tx(rpc, payer, vec![solana_sdk::instruction::Instruction {
        ...
        data: instruction::RegisterDwallet {
            args: RegisterDwalletArgs {
                chain: 0, dwallet_id: format!("btc-mock-{seed}"),
                address: "bc1qmockaddress00000000000000000000000000000".to_string(),
                balance_usd: 0, dwallet_account: None, authorized_user_pubkey: None,
                message_metadata_digest: None, public_key_hex: None, timestamp: created_at + 3,
            },
        }.data(),
    }], &[])?;

    let domain = fetch_treasury_domain(rpc, &treasury)?;
    ensure!(domain.dwallets.len() == 3, "[10] expected 3 dWallets, got {}", domain.dwallets.len());
    ensure!(domain.dwallets.contains_key(&aura_policy::Chain::Solana), "[10] SOL missing");
    ensure!(domain.dwallets.contains_key(&aura_policy::Chain::Ethereum), "[10] ETH missing");
    ensure!(domain.dwallets.contains_key(&aura_policy::Chain::Bitcoin), "[10] BTC missing");
...
async fn scenario_reputation_scaling(
    ...
    let (treasury, created_at) = setup_treasury(rpc, payer, &agent_id, aura_policy::PolicyConfig {
        daily_limit_usd: 10_000, per_tx_limit_usd: 400,
        daytime_hourly_limit_usd: 10_000, nighttime_hourly_limit_usd: 10_000,
        velocity_limit_usd: 10_000,
        ..Default::default()
    })?;
    ...
    let mut dw = connect_dwallet_client().await?;
    let domain = run_confidential_cycle(rpc, payer, treasury, encrypt_program, ep,
        10_000, 600 /* effective per_tx */, 0, 500, created_at, 2).await?;
    ensure!(domain.pending.context("[11] no pending")?.decision.approved, "[11] should be approved");
...
async fn scenario_reconfigure_guardrails(
    ...
    let (treasury, created_at) = setup_treasury(rpc, payer, &agent_id, aura_policy::PolicyConfig {
        daily_limit_usd: 200, per_tx_limit_usd: 100,
        daytime_hourly_limit_usd: 10_000, nighttime_hourly_limit_usd: 10_000,
        velocity_limit_usd: 10_000,
        ..Default::default()
    })?;
    ...
    let d1 = run_confidential_cycle(rpc, payer, treasury, encrypt_program, ep,
        200, 100, 0, 150, created_at, 2).await?;
    ensure!(!d1.pending.context("[12] no pending (1)")?.decision.approved, "[12] should be denied");
    ...
    send_tx(rpc, payer, vec![solana_sdk::instruction::Instruction {
        ...
        data: instruction::ConfigureConfidentialGuardrails { now: created_at + 30 }.data(),
    }], &[])?;
    ...
    let mut dw = connect_dwallet_client().await?;
    let d2 = run_confidential_cycle(rpc, payer, treasury, encrypt_program, ep,
        2_000, 500, 0, 150, created_at, 35).await?;
    ensure!(d2.pending.context("[12] no pending (2)")?.decision.approved, "[12] should be approved after reconfig");
    println!("  ✓ approved under new limits (per_tx=500, amount=150)");
    finalize_via_dwallet(rpc, payer, &mut dw, treasury, dwallet_program, live, created_at + 60).await
}
...
async fn main() -> anyhow::Result<()> {
    ...
    scenario_per_tx_approve(&rpc, &payer, &encrypt_program, &ep, &dwallet_program, &live, seed).await?;
    println!("  ✓ [2] passed");

    scenario_daily_deny(&rpc, &payer, &encrypt_program, &ep, seed).await?;
    println!("  ✓ [3] passed");

    scenario_daily_approve(&rpc, &payer, &encrypt_program, &ep, &dwallet_program, &live, seed).await?;
    println!("  ✓ [4] passed");
    ...
    scenario_multisig_override(&rpc, &payer, &encrypt_program, &ep, &dwallet_program, &live, seed).await?;
    println!("  ✓ [7] passed");

    scenario_swarm_pool(&rpc, &payer, &encrypt_program, &ep, &dwallet_program, &live, seed).await?;
    println!("  ✓ [8] passed");
    ...
    scenario_reputation_scaling(&rpc, &payer, &encrypt_program, &ep, &dwallet_program, &live, seed).await?;
    println!("  ✓ [11] passed");

    scenario_reconfigure_guardrails(&rpc, &payer, &encrypt_program, &ep, &dwallet_program, &live, seed).await?;
    println!("  ✓ [12] passed");
