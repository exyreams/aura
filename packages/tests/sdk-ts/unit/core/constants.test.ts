...
test("Ika devnet program ids and RPC url are valid", () => {
  // Assert validity by value (32 raw bytes + base58 round-trip) rather than
  // `instanceof PublicKey`: this suite imports the SDK from local source, which
  // resolves its own @solana/web3.js copy, so the SDK-created PublicKey is not
  // an instance of this package's PublicKey class (separate module realms).
  for (const id of [DWALLET_DEVNET_PROGRAM_ID, ENCRYPT_DEVNET_PROGRAM_ID]) {
    assert.equal(id.toBytes().length, 32);
    assert.equal(new PublicKey(id.toBase58()).toBase58(), id.toBase58());
  }
