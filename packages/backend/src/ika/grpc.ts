...
} from '../vendor/ika/generated/grpc/ika_dwallet.js';
import { defineBcsTypes } from '../vendor/ika/bcs-types.js';
...
export { defineBcsTypes } from '../vendor/ika/bcs-types.js';
...
export function createIkaClient(grpcUrl?: string, secretKey?: Uint8Array): IkaDWalletClient {
  ...
  return {
    async requestDKG(senderPubkey) {
      // Use a random session preimage so concurrent DKG calls don't collide.
      const sessionPreimage = Array.from(
        crypto.getRandomValues(new Uint8Array(32)),
      );

      // Read the current epoch from the coordinator before submitting.
      // For pre-alpha devnet the epoch is always 1 — we hard-code it here
      // because the coordinator account read requires an RPC connection that
      // the gRPC client doesn't hold. Callers that need the live epoch should
      // read it themselves and pass it in; for now 1n is correct.
      ...
      // Derive the dWallet PDA from (curve=Curve25519=2, publicKey) using the
      // same seed layout as the Rust smoke test: derive_dwallet_pda().
      // Seeds: ["dwallet", ...32-byte chunks of (u16LE(curve) ++ publicKey)]
      // We return the raw bytes here; callers use deriveDwalletPda() in protocol.ts.
      return {
        dwalletAddr: new Uint8Array(created.public_key), // placeholder — caller derives PDA
