...
export function DocsNav() {
  ...
  const pillDark = "bg-[rgba(28,28,32,0.82)] border border-[rgba(255,255,255,0.12)] shadow-[0_8px_40px_rgba(0,0,0,0.6),inset_0_1px_0_rgba(255,255,255,0.07)]";
  const pillLight = "bg-[rgba(255,255,255,0.72)] border border-[rgba(0,0,0,0.1)] shadow-[0_8px_40px_rgba(0,0,0,0.1),inset_0_1px_0_rgba(255,255,255,0.8)]";
  ...
  const MenuButton = () => (
    ...
    >
      <AnimatePresence mode="wait" initial={false}>
        {mobileOpen ? (
          <m.span key="x" className="absolute inset-0 flex items-center justify-center"
            initial={{ opacity: 0, rotate: -45, scale: 0.7 }} animate={{ opacity: 1, rotate: 0, scale: 1 }}
            exit={{ opacity: 0, rotate: 45, scale: 0.7 }} transition={{ duration: 0.18 }}>
            <X className="size-4" />
          </m.span>
        ) : (
          <m.span key="menu" className="absolute inset-0 flex items-center justify-center"
            initial={{ opacity: 0, rotate: 45, scale: 0.7 }} animate={{ opacity: 1, rotate: 0, scale: 1 }}
            exit={{ opacity: 0, rotate: -45, scale: 0.7 }} transition={{ duration: 0.18 }}>
  ...
  return (
    <>
      ...
      <div className={wrapperCls}>
        ...
        <nav className={`${navCls} hidden md:flex`}>
          ...
          <div className="flex items-center gap-5">
            ...
            <FullSearchTrigger />
            <ThemeSwitch />
            <Link href="https://github.com/aura-protocol/aura" target="_blank" rel="noopener noreferrer"
        ...
        {/* Mobile pill */}
        <div className={`md:hidden w-full overflow-hidden rounded-[14px] backdrop-blur-lg ${pill}`}>
          ...
          <AnimatePresence initial={false}>
            {mobileOpen && (
              <m.div key="body"
                ...
                <div className={`mx-3 border-t ${isDark ? "border-[rgba(255,255,255,0.08)]" : "border-[rgba(0,0,0,0.08)]"}`} />
      ...
      <AnimatePresence>
        {mobileOpen && (
          <m.div key="backdrop" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
