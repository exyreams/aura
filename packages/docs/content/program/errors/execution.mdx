...
| 6076 | `IntentDisabled` | Executing a paused scheduled intent. |
| 6077 | `IntentNotDue` | Executing a scheduled intent before its next slot. |
| 6078 | `IntentBudgetExhausted` | Per-run or lifetime budget exhausted. |
| 6079 | `IntentRunsExhausted` | The intent's run count cap is reached. |
| 6080 | `IntentExpired` | The intent passed its `end_at`. |
| 6081 | `UnauthorizedKeeper` | Caller is not the intent's authorized keeper. |
| 6082 | `InvalidIntentConfig` | Invalid recurrence / budget / condition config. |
| 6083 | `ConditionUnmet` | Trigger conditions are not satisfied. |
| 6084 | `ConditionExpired` | A conditional proposal expired unmet. |
| 6085 | `TooManyConditions` | More than the per-item condition cap. |
