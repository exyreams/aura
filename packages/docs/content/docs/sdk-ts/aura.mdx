...
```typescript
aura.treasury.cancel(options: { treasury: PublicKey }): Promise<string>
```
...
| All 50+ instructions | Subset (common ops) | Full surface |
| Best for | Apps, agents, scripts | Advanced / batching |

For anything not covered by `Aura`, drop down to `aura.lowLevel`.

<Cards>
  <Card title="AuraClient Overview" description="Full low-level API with all 50+ instructions." href="/docs/sdk-ts/client/overview" />
  <Card title="Examples" description="End-to-end patterns using both API layers." href="/docs/sdk-ts/examples" />
</Cards>
