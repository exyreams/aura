...
<Mermaid chart="sequenceDiagram;participant Agent as AI Agent;participant Core as aura-core;participant Policy as aura-policy;participant DWallet as Ika dWallet;Agent->>Core: propose_transaction(amount_usd=250, target_chain=2, recipient=0x...);Core->>Policy: evaluate_transaction() — all rules pass;Core->>Core: store PendingTransaction { status: Pending };Note over Core,DWallet: Operator drives execution...;Operator->>Core: execute_pending(dwallet, message_approval, cpi_authority, ...);Core->>DWallet: CPI — approve_message(dwallet_id, message_digest, signature_scheme_code);DWallet->>DWallet: verify AURA cpi_authority is authorized;DWallet-->>Core: MessageApproval PDA populated;Operator->>Core: finalize_execution(message_approval, ...);Core->>Core: verify signature, close PendingTransaction, update counters;Core-->>Agent: ProposalFinalized event;Agent->>Ethereum: broadcast signed transaction" />
