...
expiry, and finalization release or settle that reservation.
...
| `finalize_execution` | Verify the dWallet signature, settle scheduled-intent counters when supplied, and settle any asset-aware wallet ledger. | |
...
(price / time-window / balance / oracle flag, combined with AND/OR).
