...
On-chain standing orders (payroll / DCA / sweeps) in a `ScheduledIntent` PDA. Each run is gated by the
**same** policy + failure-mode path as an interactive proposal, plus optional **trigger conditions**
...
| `close_scheduled_intent` | Close an intent and reclaim rent. | <New /> |
| `execute_scheduled_intent` | Run a due slot (keeper or permissionless); uses the on-chain clock, advances the schedule. | <New /> |
