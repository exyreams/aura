...
| `confirm_policy_decryption` | Verify the decrypted violation code and apply it (maps `5 → WeeklyLimit`). | <New /> |
...
<Callout title="Confidentiality scope">
  The encrypted amount is not yet bound to the executed amount, and the encrypted spent counter is not read
  back across the decryption boundary — both require Encrypt-network features (a plaintext commitment and an
  exposed epoch) that the pre-alpha does not provide yet. Tracked for the decryption-boundary work.
</Callout>
