...
<Callout type="warn" title="Requires the Ika Encrypt network">
  Every instruction here except `disable`/`close` takes ciphertext accounts and
  the Encrypt CPI accounts. Derive the AURA-side authority with
  `deriveEncryptCpiAuthorityAddress()` and the event authority with
  `deriveEncryptEventAuthorityAddress(encryptProgramId)`. These flows are
  covered by the Rust smoke tests in `smoke/aura-devnet/`.
</Callout>
