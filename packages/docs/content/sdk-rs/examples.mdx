...
`ProposeTransactionArgs` accepts optional chain-native binding fields for
chain-bound proposals. For the ergonomic public flow, all binding fields are
`None`. For chain-bound flows where Ika must sign exact native bytes, supply
`native_message_hash` and the chain-specific replay fields — see the
[dWallet Execution](/overview/dwallet-execution) guide.
...
client.propose_transaction(&ai_authority, treasury, ProposeTransactionArgs {
    ...
    sanctions_proof: vec![],
    // Optional chain-native asset / binding fields (all None for legacy proposals)
    asset_id: None,
    native_amount: None,
    decimals: None,
    ...
    gas_limit: None,
    max_fee_native: None,
    native_message_hash: None, // Required for chain-bound proposals
    calldata_hash: None,
    utxo_set_hash: None,
    sighash_type: None,
