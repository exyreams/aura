...
The type-safe, uniform path for the **entire 161-instruction surface**: pair any
standalone builder from [`aura_sdk::instructions`](/sdk-rs/instructions) with
`execute`. It rewrites the instruction to the client's configured program ID,
then signs and sends.
...
```rust
use aura_sdk::instructions::policy;

let ix = policy::discard_canary(accounts);
client.execute(&owner, ix, &[])?;
```

### execute_many

Submit several builder-produced instructions atomically in one transaction,
rewriting each to the configured program ID. `execute_with_default_payer` is the
same but signs with the configured default payer.
...
- [Program API](/sdk-rs/instructions) - All instruction builders by domain
- [Types](/sdk-rs/client/types) - Rust types and domain model
- [Examples](/sdk-rs/examples) - Complete code examples
