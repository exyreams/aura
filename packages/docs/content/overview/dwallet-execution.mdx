...
description: How AURA uses Ika dWallet records to co-sign transactions across 6 chains without exposing a private key.
icon: Wallet
status: new
---
...
<Mermaid chart="flowchart TD;BTC[Bitcoin Proposal] --> Threshold{amount_usd > bitcoin_manual_review_threshold_usd?};Threshold -->|yes| Block[ViolationCode::BitcoinManualReview];Threshold -->|no| Normal[Normal execution path];Block -->|guardian calls approve_pending_execution| Override[Guardian override — proposal proceeds]" />
