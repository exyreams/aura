...
For chain-bound proposals where Ika must sign exact native bytes (Solana SPL
transfer, EVM transaction, etc.), `proposeTransaction` also accepts
`nativeMessageHash` and chain-specific replay fields. See
[dWallet Execution](/sdk-ts/execution/dwallet-execution) for the full flow.
