import type { TreasuryAccountRecord } from "@aura-protocol/sdk-ts";
...
import { createTable } from "../ui/output.js";
import {
  formatChain,
...
function renderPending(account: TreasuryAccountRecord): string | undefined {
  ...
  if (pending.decryptionRequest) {
    table.push(
      ...
      [
        "Decrypt requested",
        formatTimestamp(pending.decryptionRequest.requestedAt),
      ],
      [
        "Decrypt verified",
        formatTimestamp(pending.decryptionRequest.verifiedAt),
      ],
      [
        "Plaintext sha256",
        formatNullable(pending.decryptionRequest.plaintextSha256),
      ],
    );
  }
  ...
  if (pending.signatureRequest) {
    table.push(
      ...
      [
        "Signature requested",
        formatTimestamp(pending.signatureRequest.requestedAt),
      ],
    );
  }
  ...
  if (account.multisig) {
    ...
    table.push([
      ...
      account.multisig.guardians
        .map((guardian) => {
          const key = formatPubkey(guardian.key);
          return guardian.weight !== 1 ? `${key}·w${guardian.weight}` : key;
        })
        .join(", "),
    ]);
    if (account.multisig.pendingOverride) {
      table.push(
        [
          "Override limit",
          formatUsd(account.multisig.pendingOverride.newDailyLimitUsd),
        ],
        [
          "Override sigs",
        ...
        [
          "Override expiry",
          formatTimestamp(account.multisig.pendingOverride.expiration),
        ],
      );
    }
  }
