...
const _ENCRYPT_EVENT_AUTHORITY_SEED = Buffer.from("__event_authority");
...
function findDwalletForPending(
  ...
  const dwallet = account.dwallets.find(
    (entry) => entry.chain === pending.targetChain,
  );
  if (!dwallet) {
    throw new Error(
...
export function deriveEncryptAccounts(
  ...
  const encryptProgramId =
    options.encryptProgramId ?? ENCRYPT_DEVNET_PROGRAM_ID;
...
export function resolveScalarGuardrails(account: TreasuryAccountRecord): {
  ...
  if (
    ...
    throw new Error(
      "Scalar confidential guardrails are not configured on this treasury.",
    );
  }
  return {
...
export function resolvePendingPolicyOutput(
  account: TreasuryAccountRecord,
): PublicKey {
  const pending = resolvePendingProposal(account);
  if (!pending.policyOutputCiphertextAccount) {
    throw new Error(
      "The pending proposal does not have a confidential policy output ciphertext.",
    );
  }
  return new PublicKey(pending.policyOutputCiphertextAccount);
}

export function resolvePendingRequestAccount(
  account: TreasuryAccountRecord,
): PublicKey {
  const pending = resolvePendingProposal(account);
  ...
  if (!requestAccount) {
    throw new Error(
      "The pending proposal does not have an active decryption request.",
    );
  }
...
export function deriveApprovedExecutionAccounts(
  ...
  if (!pending.decision.approved) {
    throw new Error(
      "The current pending proposal is denied and does not require dWallet signing.",
    );
  }

  const dwalletProgramId =
    options.dwalletProgramId ?? DWALLET_DEVNET_PROGRAM_ID;
  const auraProgramId = options.auraProgramId ?? AURA_PROGRAM_ID;
  const dwallet = findDwalletForPending(account, pending);
  const dwalletAccount = getDwalletRuntimePubkey(
    dwallet.dwalletAccount,
    "dWallet runtime account",
  );
  ...
  const [dwalletCoordinator] =
    deriveDwalletCoordinatorAddress(dwalletProgramId);
...
export function buildMessageDigestHex(
  ...
  return Buffer.from(
    keccakDigest(buildPendingMessage(pending, dwallet)),
  ).toString("hex");
}
...
export function markInstructionSigner(
  ...
  if (!entry) {
    throw new Error(
      `Instruction is missing account meta for signer ${pubkey.toBase58()}`,
    );
  }
  entry.isSigner = true;
}
...
export async function sendInstructionsWithBudget(options: {
  ...
  const commitment =
    sendOptions?.preflightCommitment ?? ("confirmed" as Commitment);
...
export async function ensureEncryptDeposit(options: {
  ...
  const existing = await options.connection.getAccountInfo(
    accounts.deposit,
    "confirmed",
  );
  if (existing) {
  ...
  const configInfo = await options.connection.getAccountInfo(
    accounts.config,
    "confirmed",
  );
  if (!configInfo || configInfo.data.length < 132) {
    throw new Error(
      "Encrypt config account not found or too small for fee-vault lookup.",
    );
  }
...
export async function waitForMessageApproval(
  ...
  return await waitForAccountState(
    ...
    (account) => {
      ...
      return (
        state === targetState ||
        (targetState === "pending" && state === "signed")
      );
    },
    options,
  );
...
export function parseDecryptionReady(
  account: BufferAccountInfo | null,
): boolean {
...
export function parseCiphertextVerified(
  account: BufferAccountInfo | null,
): boolean {
  return !!account && account.data.length >= 100 && account.data[99] === 1;
}

export function buildDryRunKeypair(
