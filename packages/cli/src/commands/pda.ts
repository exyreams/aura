...
function idOption(
  ...
  return parseU64(
    optionalString(options, specificKey) ?? requireString(options, "id"),
    label,
  );
}
...
export function registerPdaCommands(program: Command): void {
  program
    ...
    .option(
      ...
      "optional 32-byte message metadata digest",
    )
    .action(function derivePda(rawKind: string) {
      ...
      const treasury = () =>
        parsePubkey(requireString(options, "treasury"), "treasury");
      ...
      switch (kind) {
        case "treasury": {
          ...
          derived = result(
            kind,
            programId,
            deriveTreasuryAddress(owner, agentId, programId),
            {
        ...
        case "message-approval": {
          ...
          const publicKey = parseHexBytes(
            requireString(options, "publicKeyHex"),
            "publicKeyHex",
          );
          ...
          const metadataValue = optionalString(
            options,
        ...
        case "exposure-group": {
          const authority = parsePubkey(
            requireString(options, "authority"),
            "authority",
          );
          const groupId = parseFixedHex(
            requireString(options, "groupId"),
            16,
            "groupId",
          );
          derived = result(
        ...
        case "operator-role": {
          const key = treasury();
          const operator = parsePubkey(
            requireString(options, "operator"),
            "operator",
          );
          derived = result(
            kind,
        ...
        case "external-liveness": {
          ...
          derived = result(
            kind,
            programId,
            deriveExternalLivenessAddress(key, programId),
            {
        ...
        case "policy-attestation": {
          const key = treasury();
          const attester = parsePubkey(
            requireString(options, "attester"),
            "attester",
          );
