import { accounts, type TreasuryAccountRecord } from "@aura-protocol/sdk-ts";
...
function DashboardApp(props: {
  ...
  }, [props.ctx, props.intervalMs, props.treasury]);
  ...
  if (!snapshot) {
    return (
      <Box flexDirection="column">
        ...
        <Text>
          {error ? `Loading failed: ${error}` : "Loading treasury state..."}
        </Text>
      </Box>
    );
  }
  ...
  return (
    <Box flexDirection="column">
      <Box marginBottom={1} flexDirection="column">
        <Text color="cyanBright">AURA Dashboard</Text>
        <Text>
          {snapshot.account.agentId} | {snapshot.treasury.toBase58()} |
          refreshed{" "}
      ...
      {sections.confidential ? (
        <Section title="Confidential" body={sections.confidential} />
      ) : null}
      {sections.pending ? (
        <Section title="Pending" body={sections.pending} />
      ) : null}
      {snapshot.livePanel ? (
        <Section title="Live" body={snapshot.livePanel} />
      ) : null}
      {sections.dwallets ? (
        <Section title="dWallets" body={sections.dwallets} />
      ) : null}
      {sections.governance ? (
        <Section title="Governance" body={sections.governance} />
      ) : null}
    </Box>
  );
}
