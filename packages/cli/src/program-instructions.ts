...
import { serializeInstruction } from "./output.js";
...
function parseArgs(
  ...
  if (Array.isArray(input)) {
    if (input.length !== instruction.args.length) {
      throw new Error(`${instruction.name} expects ${instruction.args.length} args.`);
    }
  ...
  return instruction.args.map((arg) => {
    ...
    if (value === undefined && instruction.args.length === 1 && arg.name === "args") {
      value = body;
    }
...
export async function buildProgramInstruction(
  ...
  if (typeof method !== "function") {
    throw new Error(`${instructionDef.name} is not exposed by the Anchor client.`);
  }
...
export function parseJsonInput(value: string | undefined, label: string): JsonRecord | unknown[] {
  if (!value) {
    return {};
  }
...
export function parseKeyValuePairs(values: string[] | undefined): JsonRecord {
  ...
  for (const entry of values ?? []) {
    ...
    try {
      out[key] = /^-?\d+$/.test(raw) && raw.length > 15 ? raw : (JSON.parse(raw) as unknown);
    } catch {
      out[key] = raw;
    }
  }
