/** Ika/dWallet/Encrypt derivation + on-chain account parsers. */
...
function sampleAccount(): TreasuryAccountRecord {
  return {
    ...
    confidentialGuardrails: {
      dailyLimitCiphertext: new PublicKey(
        "Stake11111111111111111111111111111111111111",
      ),
      perTxLimitCiphertext: new PublicKey(
        "Config1111111111111111111111111111111111111",
      ),
      spentTodayCiphertext: new PublicKey(
    ...
    multisig: null,
    swarm: null,
  } as unknown as TreasuryAccountRecord;
}
...
test("deriveApprovedExecutionAccounts derives message approval and dWallet CPI accounts", () => {
  const account = sampleAccount();
  const resolved = deriveApprovedExecutionAccounts(account);
  assert.equal(
    resolved.messageApproval.toBase58(),
    "DsqxVi1EZeMQGUgmLHYZTuHRkk3bUGNhFuUKJsrzYmDT",
  );
