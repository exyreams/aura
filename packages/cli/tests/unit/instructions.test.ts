/** IDL-driven instruction engine: catalog, schema, build, and overrides. */

import assert from "node:assert/strict";
import test from "node:test";
import { AURA_IDL, AURA_PROGRAM_ID, AuraClient } from "@aura-protocol/sdk-ts";
...
function sampleAccounts(
  ...
  return Object.fromEntries(
    schema.accounts.map((account) => {
      ...
      if (account.signer) return [account.name, wallet.toBase58()];
...
test("the catalog exposes every IDL instruction with a schema", () => {
...
test("every IDL instruction build-serializes with generated inputs", async () => {
  ...
  for (const instruction of getProgramInstructionCatalog().domains.flatMap(
    ...
    try {
      ...
      assert.equal(
        build.instruction.programId.toBase58(),
...
test("schema lookup accepts canonical and camelCase names", () => {
...
test("ix schema runs without wallet configuration", async () => {
  const parsed = await runCliJson<{ name: string; accounts: unknown[] }>([
    "ix",
    "schema",
